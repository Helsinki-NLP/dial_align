                ...
                for f in range(trglen):
                    ...
                    if (e, f) in alignment:
                        ...
                        for neighbor in neighbors:
                            ...
                            if (
                                e_new not in aligned['e'] or f_new not in aligned['f']
...
if __name__ == "__main__":
    ...
    tgt_file = open(sys.argv[2])
    fwd_file = open(sys.argv[3])
    rev_file = open(sys.argv[4])
    for src_line, tgt_line, fwd_line, rev_line in zip(src_file, tgt_file, fwd_file, rev_file):
        srclen = len(src_line.strip().split(" "))
        tgtlen = len(tgt_line.strip().split(" "))
        align = grow_diag_final_and(srclen, tgtlen, fwd_line.strip(), rev_line.strip())
        alignstr = " ".join(["{}-{}".format(x[0], x[1]) for x in align])
        print(alignstr)
