...
for PROJECT in archimob ndc skn; do
	echo $PROJECT
	python3 makelevpriors.py "../levenshtein/$PROJECT/*.fwd" $PROJECT.priors
	mkdir -p $PROJECT
	...
	for F in $FILES; do
		FID=`basename $F .orig`
		echo "  $FID"
		align_eflomal.py -s ../data/$PROJECT/$FID".orig" -t ../data/$PROJECT/$FID".norm" -f $PROJECT/$FID".fwd" -r $PROJECT/$FID".rev" --priors $PROJECT.priors
