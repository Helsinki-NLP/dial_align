		...
		for s in par.find_all("sentence"):
			...
			for l in lines:
				if l.strip() == "":
					continue
				e = l.split("\t")
				# column 1 contains plenty of combining diacritics that need to be taken care of specifically
				# column 2 doesn't contain any combining diacritics
				o = e[2].strip()
				o = re.sub(r'\s+', ' ', o)
				if o != "" and o != "_":
					origwords.append(o)
				# column 0 contains a single combining diacritic (by error) - we just remove it
