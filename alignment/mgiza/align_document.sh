...
$MGIZABINDIR/mgiza -ncpus $NTASKS -S $PROJECT/$FID.orig.vcb -T $PROJECT/$FID.norm.vcb -C $PROJECT/$FID.orig_$FID.norm.snt -CoocurrenceFile $PROJECT/$FID.orig_norm.cooc -o $PROJECT/$FID.align.fwd
FILES=`ls $PROJECT/$FID.align.fwd.A3.final.part*`
echo $FILES
python $MGIZABINDIR/merge_alignment.py $FILES > $PROJECT/$FID.align.fwd.A3.final

$MGIZABINDIR/mgiza -ncpus $NTASKS -S $PROJECT/$FID.norm.vcb -T $PROJECT/$FID.orig.vcb -C $PROJECT/$FID.norm_$FID.orig.snt -CoocurrenceFile $PROJECT/$FID.norm_orig.cooc -o $PROJECT/$FID.align.rev
FILES=`ls $PROJECT/$FID.align.rev.A3.final.part*`
echo $FILES
