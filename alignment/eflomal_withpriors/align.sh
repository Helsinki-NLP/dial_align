...
for PROJECT in archimob ndc skn; do
	echo $PROJECT
	cat ../data/$PROJECT/*.orig > $PROJECT"_all".orig
	cat ../data/$PROJECT/*.norm > $PROJECT"_all".norm
	...
	for F in $FILES; do
		FID=`basename $F .orig`
		echo "  $FID"
		align_eflomal.py -s ../data/$PROJECT/$FID".orig" -t ../data/$PROJECT/$FID".norm" -f $PROJECT/$FID".fwd" -r $PROJECT/$FID".rev" --priors "$PROJECT_all".priors
